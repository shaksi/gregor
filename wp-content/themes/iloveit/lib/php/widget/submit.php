		...
		if(is_numeric(options::get_value( 'upload' , 'post_item_page' )) && options::get_value( 'upload' , 'post_item_page' ) > 0){
			...
			if( options::logic( 'general' , 'user_register' ) ){
				if( is_user_logged_in () ){
					?><p><a href="<?php  echo get_page_link(options::get_value( 'upload' , 'post_item_page' ));  ?>"><?php echo $title ?></a></p><?php
				}else{
					?><p><a class="simplemodal-login  simplemodal-submit" href=""><?php echo $title ?></a></p><?php
				}
			}else{
